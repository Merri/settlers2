...
var TERRAINS = ['Greenland', 'Wasteland', 'Winter'];
var TERRAIN_COLOR = [ // Merri's custom index
...
var TERRAIN_COLORO = [ // Original game index
...
function count_texture(WLD, id, x, y, w, h)
{
	var count_c = count_cx1 = count_cy1 = 0;
	var type = texture_type(id);
	// validate locount_cations
	count_cx1 = (w + x - 1) % w;
	count_cy1 = (h + y - 1) % h;
	// count_count textures on same row
	count_c += texture_type(WLD.block[1][y * w + x]) == type;
	count_c += texture_type(WLD.block[2][y * w + count_cx1]) == type;
	count_c += texture_type(WLD.block[2][y * w + x]) == type;
	// count_count textures on row above
	if(y & 1)
	{
		x2 = (x + 1) % w;
		count_c += texture_type(WLD.block[1][count_cy1 * w + x]) == type;
		count_c += texture_type(WLD.block[1][count_cy1 * w + x2]) == type;
		count_c += texture_type(WLD.block[2][count_cy1 * w + x]) == type;
	}
	else
	{
		count_c += texture_type(WLD.block[1][count_cy1 * w + count_cx1]) == type;
		count_c += texture_type(WLD.block[1][count_cy1 * w + x]) == type;
		count_c += texture_type(WLD.block[2][count_cy1 * w + count_cx1]) == type;
	}
	// return count_count
	return count_c;
}
...
function calculate_resources(WLD) {
	...
	for(i = 0; i < WLD.size; i++)
	{
		...
		if(j >= 0x40 && j <= 0x5F)
		{
			...
			if( (j = count_texture(WLD, 1, x = (i % w), y = (((i-j) / w) | 0), w, h)) < 6)
			{
				// is not
				WLD.block[11][i] = 0;
			}
	...
	for(y = 0; y < h; y++)
	{
		...
		for(x = 0; x < w; x++)
		{
			...
			if( WLD.block[8][i] == 0x09 )
			{
				...
				if(y & 1)
				{
					if( count_texture(WLD, 5, x, y1, w, h) == 6)
					{
						WLD.block[11][y1 * w + x] = 0x87;
					}
					if( count_texture(WLD, 5, x2, y1, w, h) == 6)
					{
						WLD.block[11][y1 * w + x2] = 0x87;
					}
					if( count_texture(WLD, 5, x, y2, w, h) == 6)
					{
						WLD.block[11][y2 * w + x] = 0x87;
					}
					if( count_texture(WLD, 5, x2, y2, w, h) == 6)
					{
						WLD.block[11][y2 * w + x2] = 0x87;
					}
				}
				else
				{
					if( count_texture(WLD, 5, x1, y1, w, h) == 6)
					{
						WLD.block[11][y1 * w + x1] = 0x87;
					}
					if( count_texture(WLD, 5, x, y1, w, h) == 6)
					{
						WLD.block[11][y1 * w + x] = 0x87;
					}
					if( count_texture(WLD, 5, x1, y2, w, h) == 6)
					{
						WLD.block[11][y2 * w + x1] = 0x87;
					}
					if( count_texture(WLD, 5, x, y2, w, h) == 6)
					{
						WLD.block[11][y2 * w + x] = 0x87;
				...
				if( count_texture(WLD, 5, x1, y, w, h) == 6)
				{
					WLD.block[11][y * w + x1] = 0x87;
				}
				if( count_texture(WLD, 5, x2, y, w, h) == 6)
				{
					WLD.block[11][y * w + x2] = 0x87;
				}
			}
			// all meadow?
			else if( count_texture(WLD, 0, x, y, w, h) == 6)
			{
				// water here
				WLD.block[11][i] = 0x21;
			}
...
function MerrisGouraud(ev) {
	...
	for(y = 0; y < h; y++) {
		...
		for(x = 0; x < w; x++) {
			...
			j = (i * 768 + TERRAIN_COLOR[WLD.type][ WLD.block[1][k] & 0x1F ] + tk) * 4;
			j2 = (i * 768 + TERRAIN_COLORO[WLD.type][ WLD.block[1][kp1] & 0x1F ] + tk) * 4;
			l = (i * 768 + TERRAIN_COLORO[WLD.type][ WLD.block[2][k] & 0x1F ] + tk) * 4;
			l2 = (i * 768 + TERRAIN_COLOR[WLD.type][ WLD.block[2][kp1] & 0x1F ] + tk) * 4;
