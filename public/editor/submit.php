...
if(isset($_POST['areas']) && is_array($_POST['areas']))
{
	foreach($_POST['areas'] as $area)
	{
		if( is_array($area) && isset($area['id']) && isset($area['x']) && isset($area['y']) && isset($area['mass'])
			&& isset($area['continent']) && isset($area['harbors']) )
		{
			$areas[] = array(
				'id' => intval($area['id']),
				'x' => intval($area['x']),
...
$harbors = array();
if(isset($_POST['harbors']) && is_array($_POST['harbors']))
{
	foreach($_POST['harbors'] as $harbor)
	{
		if( is_array($harbor) && isset($harbor['node']) && isset($harbor['x']) && isset($harbor['y']) )
		{
			$harbor_areas = array();
			if( isset($harbor['areas']) && is_array($harbor['areas']) )
			{
				foreach($harbor['areas'] as $harbor_area)
				{
					$harbor_areas[] = array(
						'area' => intval($harbor_area['area']),
						'node' => intval($harbor_area['node'])
					);
				}
			}
			$harbors[] = array(
				'node' => intval($harbor['node']),
				'x' => intval($harbor['x']),
				'y' => intval($harbor['y']),
				'areas' => $harbor_areas
			);
...
if($width < 0 || $width > 256 || ($width & 0x03) !== 0 ) $errors[] = 'width_invalid '.$width;
if($height < 0 || $height > 256 || ($height & 0x03) !== 0) $errors[] = 'height_invalid '.$height;
...
else
{
	...
	elseif(strlen($id) === 0 || is_valid_id($id, $rttr) === false)
	{
		// rank this as a new map, generate a new id
		$id = new_id($title, $rttr);
	}
	else
	{
		...
		else
		{
			...
			if(count($harbors) !== $maps[0]['map_harbors']) $notices[] = 'map_harbors_change '.$maps[0]['map_harbors'];
		}
	}
}
