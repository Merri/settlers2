	...
	if( (!DAT && ($filesize > 16911)) || (DAT && ($filesize > 15433)) )
	{
			...
			&& ($header['width'] < 257) && ($header['height'] < 257) )
		{
			...
			if($handlep)
			{
				...
				if($gouraud)
				{
					$worldmap = imagecreatetruecolor($header['width'] * 2, $header['height'] * 2);
					$palindex = array();
...
/*
								...
								$pos = array(
									$goumap[ $p1 = ord($mapdata[$j + $ok]) & 127 ],
									$goumap[ $p2 = ord($mapdata[$j + $ol]) & 127 ],
									$goumap[ $p3 = ord($mapdata[$kx]) & 127 ],
									$goumap[ $p4 = ord($mapdata[$j + $k]) & 127 ],
									$goumap[ $p5 = ord($mapdata[$j + $l]) & 127 ],
									$goumap[ $p6 = ord($mapdata[$x + $l]) & 127 ],

									$goumap[ $p7 = ord($mapdata[$x + $ok]) & 127 ],
									$goumap[ $p8 = ord($mapdata[$x + $ol]) & 127 ],
									$goumap[ $p9 = ord($mapdata[$x + $k]) & 127 ],
...
*/
								...
								$gou = floor((ord($mapdata[$j + $m]) * 4 + ord($mapdata[$x + $m])) / 5);
								...
								if($red) $red = floor($red / 4);
								if($green) $green = floor($green / 4);
								if($blue) $blue = floor($blue / 4);
								imagesetpixel($worldmap, $j * 2, $i * 2, imagecolorallocate($worldmap, $red, $green, $blue) );
								...
								if($green) $green = floor($green / 4);
								if($blue) $blue = floor($blue / 4);
								imagesetpixel($worldmap, $j * 2, $i * 2 + 1, imagecolorallocate($worldmap, $red, $green, $blue) );
								...
								if($green) $green = floor($green / 4);
								if($blue) $blue = floor($blue / 4);
								imagesetpixel($worldmap, $j * 2 + 1, $i * 2, imagecolorallocate($worldmap, $red, $green, $blue) );
								...
								if($green) $green = floor($green / 4);
								if($blue) $blue = floor($blue / 4);
								imagesetpixel($worldmap, $j * 2 + 1, $i * 2 + 1, imagecolorallocate($worldmap, $red, $green, $blue) );
							}
						}
						else
						{
...
/*
								...
								$pos = array(
									$goumap[ $p1 = ord($mapdata[$kx]) & 127 ],
									$goumap[ $p2 = ord($mapdata[$lx]) & 127 ],
									$goumap[ $p3 = ord($mapdata[$j + $ok]) & 127 ],
									$goumap[ $p4 = ord($mapdata[$j + $k]) & 127 ],
									$goumap[ $p5 = ord($mapdata[$j + $l]) & 127 ],
									$goumap[ $p6 = ord($mapdata[$x + $l]) & 127 ],
...
*/
								...
								$gou = floor((ord($mapdata[$j + $m]) * 5 + ord($mapdata[$x2 + $m])) / 6);
								
								$pos = array(
									$goumap[ $p1 = ord($mapdata[$j + $ok]) & 127 ],
									$goumap[ $p6 = ord($mapdata[$j + $l]) & 127 ],
									$goumap2[ $p1 ],
									$goumap2[ $p6 ],
								);
								$red = $green = $blue = 0;
								...
								$pos = array(
									$goumap[ $p4 = ord($mapdata[$x2 + $k]) & 127 ],
									$goumap[ $p6 = ord($mapdata[$j + $l]) & 127 ],
									$goumap2[ $p4 ],
									$goumap2[ $p6 ],
								);
